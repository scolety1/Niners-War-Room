...
export function AdpProvidersPage({ client, data, onUpdate }: { client: NwrApiClient; data: RedraftBootstrap; onUpdate: (data: RedraftBootstrap) => void }) {
  ...
  const approveCandidate = async (row: Record<string, unknown>, candidate: Record<string, unknown>) => {
    ...
    try {
      const next = await client.approveRedraftOwnerPlatformManualMatch(activeProfile.profileId, String(row.player_name || ""), String(row.position || ""), String(row.position_rank || ""), String(candidate.player_id || ""), snapshot?.rawHash || "");
      onUpdate(next);
  ...
  return <div className="adp-providers-page" aria-busy={Boolean(working)}>
    ...
    <div className="adp-provider-grid">
      <Panel title="Global Owner Platform Snapshot" eyebrow="Paste once · use across Redraft leagues">
        ...
        {pastePreview ? <div className="copy-muted"><div className="metric-grid"><div><strong>{pastePreview.sourceRows}</strong><small>Parsed rows</small></div><div><strong>{pastePreview.matchedRows}</strong><small>Matched</small></div><div><strong>{pastePreview.unmatched.length}</strong><small>Unmatched</small></div><div><strong>{pastePreview.rows.filter((row) => String(row.match_source || "") === "OWNER_APPROVED").length}</strong><small>Owner-approved</small></div></div><strong>{pastePreview.parserMode === "PLAIN_TEXT_BLOCK" ? "Plain-text fallback" : "Markdown table"} normalized preview</strong><small>{platformCoverageText(pastePreview.platformCoverage)}</small><small>Active league: {activeProfile?.leagueName || "—"} · Detected platform: {detectedPlatformLabel} · Active ADP column: {activeColumn} · Fallback: {activeColumn} → Consensus → FFC → Unavailable</small><div className="profile-edit-actions"><Button variant={previewFilter === "ALL" ? "primary" : "secondary"} onClick={() => { setPreviewFilter("ALL"); setPreviewLimit(25); }}>All rows</Button><Button variant={previewFilter === "MATCHED" ? "primary" : "secondary"} onClick={() => { setPreviewFilter("MATCHED"); setPreviewLimit(25); }}>Matched</Button><Button variant={previewFilter === "UNMATCHED" ? "primary" : "secondary"} onClick={() => { setPreviewFilter("UNMATCHED"); setPreviewLimit(25); }}>Unmatched</Button><Button variant={previewFilter === "AMBIGUOUS" ? "primary" : "secondary"} onClick={() => { setPreviewFilter("AMBIGUOUS"); setPreviewLimit(25); }}>Ambiguous</Button><Button variant={previewFilter === "MISSING" ? "primary" : "secondary"} onClick={() => { setPreviewFilter("MISSING"); setPreviewLimit(25); }}>Missing active platform</Button></div><div className="draft-board-scroll"><table><thead><tr><th>Row</th><th>Pos rank</th><th>Player</th><th>Consensus</th><th>Sleeper</th><th>ESPN</th><th>FantasyPros</th><th>Match status / source</th><th>Matched NWR player</th><th>Reason / review</th></tr></thead><tbody>{previewRows.slice(0, previewLimit).map((row) => <tr key={String(row.source_row_index)}><td>{String(row.source_row_index ?? "—")}</td><td>{String(row.position_rank ?? row.position ?? "—")}</td><td>{String(row.player_name ?? "—")}</td><td>{String(row.consensus_adp ?? "—")}</td><td>{String(row.sleeper_adp ?? "—")}</td><td>{String(row.espn_adp ?? "—")}</td><td>{String(row.fantasypros_adp ?? "—")}</td><td>{String(row.match_status ?? "—")} · {String(row.match_source ?? "UNMATCHED")}</td><td>{String(row.matched_nwr_player_name ?? "—")}</td><td>{String(row.unmatched_reason ?? (row.selected_adp == null ? "Selected column missing; Consensus/FFC fallback may apply" : "Matched"))}{Array.isArray(row.candidate_suggestions) ? row.candidate_suggestions.map((value) => { const candidate = value as Record<string, unknown>; return <div key={String(candidate.player_id)}><small>{String(candidate.player_name)} · {String(candidate.position)} · {String(candidate.team || "—")} · {String(candidate.confidence)}</small><Button disabled={Boolean(working) || String(candidate.position) !== String(row.position)} variant="secondary" onClick={() => void approveCandidate(row, candidate)}>Approve ADP-only match</Button></div>; }) : null}</td></tr>)}</tbody></table></div>{previewRows.length > previewLimit ? <Button variant="secondary" onClick={() => setPreviewLimit((value) => value + 25)}>Show 25 more</Button> : null}{[...pastePreview.warnings, ...pastePreview.unmatched].slice(0, 12).map((warning) => <small key={warning}>{warning}</small>)}</div> : null}
