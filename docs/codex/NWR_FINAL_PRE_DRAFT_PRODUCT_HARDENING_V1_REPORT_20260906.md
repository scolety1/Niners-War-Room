...
Full suite, run twice: **3867/334/71**, then **3874/327/71** (passed/failed/skipped). Neither the failed nor passed count is stable run-to-run in this suite (already documented and traced earlier this session as real, pre-existing flakiness unrelated to any specific change) — but in BOTH runs, the full list of failing test names is drawn from the same established cluster (data-hygiene/UI-contract reports, launcher tests) and contains zero overlap with anything touched today (`desktop_facade.py`, `decision_bundle_service_v2.py`, `decision_bundle_live_service_v2.py`, `raw_action_value_live_service.py`).
