...
page_header(
    "Drafting Mode moved into Draft Cockpit",
    eyebrow="Compatibility Route",
    description=(
        "The draft cockpit is now the Draft Cockpit page. This route stays available so old "
        "links do not break, but it no longer acts as a separate main workspace."
    ),
    status_items=(
        ("Draft Cockpit is primary", "safe"),
...
st.warning(
    "Drafting Mode is no longer a separate main nav item. Use Draft Cockpit for the live "
    "command center, runtime state, trade events, export/import, and pick-by-pick workflow."
)

cols = st.columns(3)
cols[0].link_button("Open Draft Cockpit", "/draft-cockpit", use_container_width=True)
...
st.markdown(
    """
    - **Draft Cockpit** is now the command-center draft room.
    ...
    - **Cheat Sheets / Tier Board** remain available by direct URL and will be demoted into
      Draft Cockpit and Dynasty Rankings views.
    """
)
