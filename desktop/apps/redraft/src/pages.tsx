...
export function DraftRoomPage({ client, data, onUpdate }: { client: NwrApiClient; data: RedraftBootstrap; onUpdate: (data: RedraftBootstrap) => void }) {
  ...
  const manualColumns: TableColumn[] = [
    ...
    { key: "action", label: "", align: "right", render: (row) => <Button data-draft-action disabled={Boolean(working)} variant="secondary" onClick={() => void mark(String(row.playerId))}>{working === String(row.playerId) ? "Saving…" : "Draft"}</Button> },
  ];
  return <div aria-busy={Boolean(working)} className="draft-room-page">
    ...
    <section className="on-clock"><div><span>{data.activeProfile.leagueName}</span><strong>Pick {drafted.length + 1}</strong><small>{draftFormat(data.activeProfile)}</small></div><div className="clock-ring"><strong>∞</strong><span>Offline</span></div><div><span>{manualPosition ? "Manual position" : "Top available"}</span><strong>{manualPosition ? "NWR does not model K/DST" : filteredRows[0]?.playerName ? String(filteredRows[0].playerName) : "No admitted player"}</strong><small>{manualPosition ? "Choose any available real asset" : filteredRows[0]?.position ? `${String(filteredRows[0].position)}${String(filteredRows[0].positionRank)} · Tier ${String(filteredRows[0].tier)}` : "Projection evidence required"}</small></div></section>
