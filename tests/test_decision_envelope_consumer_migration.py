...
def test_kdst_streamer_returns_a_real_decision_envelope_per_position(
    ...
    # league's real roster.k/roster.dst slot counts (never recommends a
    # position the league doesn't actually use) -- this preset defaults to
    # 0/0, so a real K/DST-using league must set them explicitly here.
    save_profile(
        store,
        replace(
