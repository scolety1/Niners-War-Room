...
> **Correction (see `docs/codex/NWR_PRACTICE_DRAFT_FINAL_RECONCILIATION_20260905.md`, section 2)**: "4-team" here is independently re-verified as the real, actual `LeagueProfile.team_count` used throughout — not a draft-slot/league-size mixup. But characterizing it as "this dataset's real, verified historical league format" was overstated: it is an unexplained, undocumented cap in the analysis pipeline (`min(4, ...)`), not a property of the underlying historical data, which could easily have supported a much larger simulated league. The practical decision below (don't wire F4 into KHA) is reinforced, not undermined, by the fuller mechanical analysis in that report — most of F4's features are genuinely not portable to a 16-team league regardless of why 4 was originally chosen.
