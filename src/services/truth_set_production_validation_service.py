...
def validate_truth_set_production_export(
    ...
    *,
    ...
    while parsed_rows and all(not str(cell).strip() for cell in parsed_rows[0]):
        leading_blank_flags.append(
            _flag(
                ...
                "",
                "",
                "Strict production exports must not contain blank rows before the header.",
            )
        )
    ...
    if header != TRUTH_SET_PRODUCTION_STRICT_HEADER:
        flags.append(
            _flag(
                source_file,
                1,
                ...
                "header",
                "|".join(header),
                "Header must exactly match the strict production schema.",
            )
        )

    for row_number, raw_row in enumerate(data_rows, start=2):
        player_name = raw_row[0] if raw_row else ""
        if len(raw_row) != len(TRUTH_SET_PRODUCTION_STRICT_HEADER):
            flags.append(
                _flag(
                    source_file,
                    row_number,
                    ...
                    (
                        ...
                        f"{len(TRUTH_SET_PRODUCTION_STRICT_HEADER)}."
                    ),
                )
            )
...
def write_production_validation_flags(
    output_path: str | Path,
    flags: tuple[dict[str, object], ...],
) -> None:
    _write_dicts(output_path, PRODUCTION_FLAG_HEADER, flags)


def write_production_validation_summary(
    output_path: str | Path,
    summaries: tuple[dict[str, object], ...],
) -> None:
    _write_dicts(output_path, PRODUCTION_SUMMARY_HEADER, summaries)


def write_production_validation_summary_json(
    output_path: str | Path,
    summaries: tuple[dict[str, object], ...],
) -> None:
...
def _result(
    ...
    summary = {
        ...
        "header_status": (
            "exact" if tuple(header) == TRUTH_SET_PRODUCTION_STRICT_HEADER else "mismatch"
        ),
        "rows": len(data_rows),
    ...
    for key, player in sorted(expected_by_key.items(), key=lambda item: item[1]):
        ...
        flags.append(
            _flag(
                ...
                "player_name",
                "",
                "Strict production export must include one row per truth-set player.",
            )
        )
        ...
        if player_keys.count(player) > 1:
            flags.append(
                _flag(
                    ...
                    "player_name",
                    player,
                    "Strict production export must have one row per player.",
                )
            )
...
def _row_dict(raw_row: list[str]) -> dict[str, str]:
    return {
        ...
        for index, column in enumerate(TRUTH_SET_PRODUCTION_STRICT_HEADER)
    }
