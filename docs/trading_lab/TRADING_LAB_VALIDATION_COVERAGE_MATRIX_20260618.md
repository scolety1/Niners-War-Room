...
| --- | --- | --- | --- | --- | --- | --- |
| Source inventory | Source inventory template, validation examples, T6 validator inventory, T7 coverage | `validate_source_metadata`, `validate_research_config`, T3/T4/T5/T6/T7 tests | Prohibited categories, broker/API language, secret-like config fields | Ready for manual review | No persisted manifest approved | Docs-only manifest acceptance criteria |
