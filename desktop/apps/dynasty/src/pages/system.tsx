...
export function DataHealthPage({ data, onReload }: { data: DynastyBootstrap; onReload: () => void }) {
  ...
  return <>
    ...
    <div className="split-view"><Panel title="Source integrity" eyebrow="Pinned hashes · no path exposure">{sourceRows.length ? <DataTable columns={[{key:"source",label:"Authority"},{key:"hash",label:"SHA-256",render:(row)=><code className="hash-value">{String(row.hash)}</code>},{key:"state",label:"State",render:()=> <StatusBadge tone="safe" label="Verified" />}]} rows={sourceRows} rowKey={(row)=>String(row.source)} /> : <p className="copy-muted">Source hash detail was not included in this runtime snapshot.</p>}</Panel><Panel title="Runtime boundary" eyebrow="Windows desktop"><dl className="health-list"><div><dt>Transport</dt><dd>127.0.0.1 only</dd></div><div><dt>Cloud dependency</dt><dd>None</dd></div><div><dt>Provider calls</dt><dd>Disabled</dd></div><div><dt>Streamlit fallback</dt><dd>Preserved</dd></div><div><dt>Mode isolation</dt><dd>Dynasty only</dd></div></dl></Panel></div>
