...
export function DraftRoomPage({ client, data, onUpdate }: { client: NwrApiClient; data: RedraftBootstrap; onUpdate: (data: RedraftBootstrap) => void }) {
  ...
  return <div aria-busy={Boolean(working)} className="draft-room-page">
    ...
    {board?.configured ? <>
      ...
      <Panel title="Draft recommendations" eyebrow="Five distinct decision lenses"><div className="recommendation-grid">{board.recommendations?.map((card) => <article key={card.label}><span>{card.label}</span><strong>{card.playerName}</strong><small>{card.position} · NWR #{card.nwrRank} · {card.rosterFit}</small><b>{card.nwrView} · {card.draftTiming}</b><p>{card.adpExplanation || "ADP unavailable · no active match"}</p><p>{card.note}</p><Button data-draft-action disabled={!ownerTurn || Boolean(working)} variant="secondary" onClick={() => void mark(card.playerId)}>Draft</Button></article>)}</div></Panel>
      <Panel title="Draft board" eyebrow={`${data.activeProfile.draft.rounds} rounds · click a team to inspect its roster`}><div className="draft-board-scroll"><div className="draft-board-grid"><div className="draft-board-corner">Rd</div>{Array.from({ length: data.activeProfile.teamCount }, (_, index) => index + 1).map((slot) => <button className={slot === board.ownerSlot ? "owner-team" : ""} key={`head-${slot}`} onClick={() => setSelectedTeam(slot)}>T{slot}{slot === board.ownerSlot ? " · YOU" : ""}</button>)}{Array.from({ length: data.activeProfile.draft.rounds }, (_, index) => index + 1).flatMap((round) => [<b className="round-label" key={`round-${round}`}>{round}</b>, ...Array.from({ length: data.activeProfile!.teamCount }, (_, column) => { const slot = column + 1; const cell = board.boardCells?.find((item) => item.round === round && item.teamSlot === slot); return <article className={`${cell?.ownerPick ? "owner-pick" : ""} ${cell?.current ? "current-pick" : ""}`} key={`${round}-${slot}`}><small>{cell?.pickNumber}</small>{cell?.playerName ? <><strong>{cell.playerName}</strong><span>{cell.position} · {cell.team}</span></> : <em>Open</em>}</article>; })])}</div></div></Panel>
