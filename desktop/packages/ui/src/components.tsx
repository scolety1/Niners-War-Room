...
export function StatusBadge({ tone, label, pulse = false }: { tone: HealthTone | "safe"; label: string; pulse?: boolean }) { return <span className={`status-badge status-badge--${tone}`}><i className={pulse ? "pulse" : ""} />{label}</span>; }
...
export function SearchInput({ value, onChange, placeholder = "Search players…" }: { value: string; onChange: (value: string) => void; placeholder?: string }) { return <label className="search-input"><Icon name="search" size={16} /><input aria-label={placeholder.replace(/…$/, "")} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} type="search" value={value} /></label>; }
