...
| docs traceability | T14 doc matrix, guardrail matrix, style guide | Full focused Trading Lab validation before commit | Docs-only traceability matrices | N/A | N/A | Docs-only traceability is not runtime enforcement | Refresh after new runway docs/tests |
| validator edge cases | T15 plan and tests | T15 edge-case tests | Recursive in-memory text traversal, punctuation-tolerant patterns | Nested prohibited text and mixed-case terms rejected | Nested config strings checked | Broker/API/order/key punctuation variants rejected | No persisted parser approved | Keep tests inline with fake payloads |
| fake example corpus | T16 fake corpus v2 | T16 regression tests | Existing validators across all supported artifact examples | Valid, invalid, and HOLD fake examples | Secret-like examples rejected | Broker/API/order examples rejected | No external corpus file approved | Keep examples inline and fictional |
| no-advice rewrites | T17 safe rewrite library and taxonomy | T17 no-advice rewrite tests | `validate_artifact_text_fields` | Advice, guaranteed return, order, and auto-execute wording rejected | Credential/private-account examples rejected | Broker token/API key examples rejected | Rewrites are guidance, not recommendations | Keep rewrites research-only |
