"""NWR Overnight V3 strategic-model-validation resume, section 2 (bench
depth extension).

`run_mru_walk_forward_v2.py`'s own 51-observation result uses the frozen
historical corpus's FIXED roster shape (QB1/RB1/WR1/TE1/FLEX1/BENCH1 --
only 6 rounds, only ONE bench slot total). That shape cannot exercise
`marginal_roster_utility_v2`'s actual design differentiator at all --
its whole real contribution is re-pricing BENCH DEPTH 2+ (WR4-WR8,
RB3-RB5), which literally cannot occur when a draft only has one bench
slot to fill. This script reuses the SAME proven bridge mechanism
(`mru_bridge_worker.py`, parity-proven in `run_mru_walk_forward_v2.py`)
against a larger, still-real, still-governed roster shape built from
the SAME historical point-in-time corpus, so the depth-4/5/6 region
where v1 and v2 actually diverge gets real draft rounds to appear in.

Roster: QB1/RB2/WR2/TE1/FLEX1/BENCH4 = 11 rounds (K/DST excluded -- this
historical dataset has no K/DST rows, same reason the frozen corpus's
own default roster excludes them). team_count=10 only (matches Test 18's
real league shape; higher team counts would need more real players per
season than several development seasons have -- disclosed, not forced).
2012 (84 real rows) is infeasible at 110 needed players and is skipped,
consistent with the frozen corpus builder's own precedent of skipping
infeasible (season, team_count) combinations rather than padding.
"""
...
OUT_PATH = THIS_BRANCH_ROOT / "docs/codex/overnight_v3/harness_v1/mru_walk_forward_bigroster_v1_report.json"
...
TEAM_COUNT = 10
BASE_SEED = 20260905
BIG_ROSTER = RosterSettings(qb=1, rb=2, wr=2, te=1, flex=1, superflex=0, k=0, dst=0, bench_size=4)
TOTAL_ROUNDS = (
