...
import { Button, DataTable, EmptyState, ErrorState, MetricCard, PageHeader, Panel, SearchInput, SelectField, StatusBadge, formatNumber } from "@nwr/ui";
...
export function PlayerDetailBody({ detail: sourceDetail, onCompare, onPersonal, onTrade }: { detail: PlayerDetail; onCompare: () => void; onPersonal: () => void; onTrade: () => void }) {
  const detail = {
    ...sourceDetail,
...
export function RookieReviewPage({ data }: { data: DynastyBootstrap }) {
  ...
  const rows = useMemo(() => data.rookies.filter((row) => (position === "ALL" || row.position === position) && (scoreView === "ALL" || row.modelScoreEligible) && matchesPlayerSearch([row.player, row.team, row.playerId, row.scoreStatus], query)), [data.rookies, position, query, scoreView]);
  const positions = ["ALL", ...new Set(data.rookies.map((row) => row.position).filter(Boolean))];
  return <>
    <PageHeader eyebrow="2026 class · Review authority" title="Rookie Intelligence Board" description="Every official drafted QB/RB/WR/TE remains draftable even when the frozen Rookie Review cannot score them." status={<><StatusBadge tone="safe" label={`${data.rookieReadiness.officialDrafted} official assets`} /><StatusBadge tone="review" label={`${data.rookieReadiness.manualReview} manual review`} /></>} />
    ...
    <Panel title="2026 prospect board" eyebrow="Draft range · evidence · research">
      <div className="toolbar"><SearchInput value={query} onChange={setQuery} placeholder="Find a rookie…" /><div className="chip-row">{positions.map((value) => <button className={`filter-chip ${position === value ? "filter-chip--active" : ""}`} key={value} onClick={() => setPosition(value)}>{value}</button>)}</div><SelectField label="Score coverage" value={scoreView} onChange={setScoreView} options={[{ value: "ALL", label: "All draftable rookies" }, { value: "SCORED", label: "Show scored only" }]} /></div>
      <DataTable columns={[
        ...
        { key: "reviewScore", label: "Review score", align: "right", render: (row) => row.modelScoreEligible ? formatNumber(row.reviewScore as number, 2) : "â€”" },
        ...
        { key: "draftRange", label: "Draft range / tier", render: (row) => <OwnerValue value={`${row.draftRange} · ${row.rookieTier}`} /> },
        { key: "eligibilityReason", label: "Key reason", render: (row) => <span className={!row.modelScoreEligible ? "warning-copy" : ""}><OwnerValue value={row.eligibilityReason || row.warnings} /></span> },
