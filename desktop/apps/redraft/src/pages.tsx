...
export function DraftRoomPage({ client, data, onUpdate }: { client: NwrApiClient; data: RedraftBootstrap; onUpdate: (data: RedraftBootstrap) => void }) {
  ...
  const [mode, setMode] = useState<"MOCK" | "LIVE_READ_ONLY">("MOCK");
  ...
  const matchesPosition = (rowPosition: string) => position === "ALL" || rowPosition === position || (position === "FLEX" && (rowPosition === "RB" || rowPosition === "WR" || rowPosition === "TE"));
  const filteredRows = rankingRows(data).filter((row) => !row.drafted && matchesPosition(String(row.position)) && (!query || String(row.playerName).toLowerCase().includes(query.toLowerCase())));
  ...
  const mark = async (playerId: string) => {
    ...
    if (liveMode) {
      const nextPick = (drafted.length ?? 0) + 1;
      const recordingTeam = board?.currentTeamSlot ?? "?";
      await mutate(playerId, () => client.ingestSleeperDraftPick(data.activeProfileId!, playerId, nextPick), `Recorded ${playerName} to Team ${recordingTeam} (pick ${nextPick}).`);
      return;
    }
  ...
  const start = async () => {
    ...
    await mutate("start", () => client.startDraftRoom(data.activeProfileId!, slot, speed, 20260817, mode), `Draft Room started from slot ${slot} in ${mode === "LIVE_READ_ONLY" ? "Live Draft" : "Mock Draft"} mode.`);
  };
  ...
  const manualColumns: TableColumn[] = [
    ...
    { key: "action", label: "", align: "right", render: (row) => <Button data-draft-action disabled={!canRecordPick || Boolean(working)} variant="secondary" onClick={() => void mark(String(row.playerId))}>{working === String(row.playerId) ? "Saving…" : "Draft"}</Button> },
  ];
  const globalSearchColumns: TableColumn[] = [
    ...
    { key: "source", label: "Status", sort: "text", render: (row) => row.source === "MANUAL" ? <StatusBadge tone="review" label="Manual · not modeled" /> : <StatusBadge tone="safe" label="NWR ranked" /> },
    { key: "action", label: "", align: "right", render: (row) => <Button data-draft-action disabled={!canRecordPick || Boolean(working)} variant="secondary" onClick={() => void mark(String(row.playerId))}>{working === String(row.playerId) ? "Saving…" : "Draft"}</Button> },
  ];
  return <div aria-busy={Boolean(working)} className="draft-room-page">
    ...
    <Panel title={trimmedQuery ? "Search results (all positions)" : manualPosition ? "Manual position" : "Available player panel"} eyebrow={trimmedQuery ? `Searching every draftable player and manual K/DST asset, position filter ignored · ${globalSearchRows.length} matches` : manualPosition ? "NWR does not currently model K/DST" : `NWR rank remains authoritative · ${filteredRows.length} matches`}><div className="toolbar"><SearchInput value={query} onChange={setQuery} /><SegmentedControl label="Position" options={DRAFT_ROOM_POSITION_OPTIONS} value={position} onChange={setPosition} />{!manualPosition ? <SelectField label="Show" value={depth} onChange={setDepth} options={DRAFT_DEPTH_OPTIONS} /> : null}</div>{trimmedQuery ? <DataTable columns={globalSearchColumns} rows={globalSearchRows as unknown as Array<Record<string, unknown>>} rowKey={(row) => String(row.playerId)} /> : manualPosition ? <DataTable columns={manualColumns} rows={manualRows as unknown as Array<Record<string, unknown>>} rowKey={(row) => String(row.playerId)} /> : <DataTable columns={[{ key: "overallRank", label: "NWR", sort: "number" }, { key: "playerName", label: "Player", sort: "text", render: (row) => <span className="player-cell"><strong>{String(row.playerName)}</strong><small>{String(row.team)} · {String(row.position)} · {String(row.positionTierLabel)}</small></span> }, { key: "overallAdp", label: "ADP", sort: "number", render: (row) => row.overallAdp == null ? "—" : formatNumber(row.overallAdp as number, 1) }, { key: "expectedRound", label: "Exp rd", sort: "number", render: (row) => row.expectedRound == null ? "—" : `R${String(row.expectedRound)}` }, { key: "nwrAdpGap", label: "Gap", sort: "number", render: (row) => row.nwrAdpGap == null ? "—" : formatNumber(row.nwrAdpGap as number, 1) }, { key: "valueLabel", label: "NWR view", sort: "text" }, { key: "timingLabel", label: "Timing", sort: "text" }, { key: "makeItBack", label: "Make it back", sort: "text" }, { key: "fp", label: "FP", sort: "text", render: (row) => intelFor(String(row.playerId))?.fantasyProsEcr ?? "—" }, { key: "udk", label: "UDK", sort: "text", render: (row) => { const entry = intelFor(String(row.playerId)); return entry?.udkPositionRank ? `${String(row.position)}${entry.udkPositionRank}` : "—"; } }, { key: "udkTier", label: "Tier", sort: "text", render: (row) => intelFor(String(row.playerId))?.udkTier ?? "—" }, { key: "udkRisk", label: "Risk", sort: "text", render: (row) => intelFor(String(row.playerId))?.udkRisk ?? "—" }, { key: "udkUpside", label: "Up", sort: "text", render: (row) => intelFor(String(row.playerId))?.udkUpside ?? "—" }, { key: "alert", label: "Alert", sort: "text", render: (row) => { const entry = intelFor(String(row.playerId)); return entry?.currentAlert ? <span title={entry.currentAlert}>{entry.currentAlertSeverity ?? "•"}</span> : "—"; } }, { key: "action", label: "", align: "right", render: (row) => <Button data-draft-action disabled={!canRecordPick || Boolean(working)} variant="secondary" onClick={() => void mark(String(row.playerId))}>{working === String(row.playerId) ? "Saving…" : "Draft"}</Button> }]} rows={rows} rowKey={(row) => String(row.playerId)} />}</Panel>
    {externalIntel && !externalIntel.available ? <p className="boundary-note">EXTERNAL INTEL UNAVAILABLE — NWR and ESPN continue to work normally.</p> : null}
